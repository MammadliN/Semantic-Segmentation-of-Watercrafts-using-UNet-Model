...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 24 16:28:59 2023
...
def read_train_img(img_path, mask_path, SIZE_Y, SIZE_X, color=0, ending="*.jpg", mask=False):
    ...
    train_images = []
    for directory_path in glob.glob(img_path):
        for img_path in glob.glob(os.path.join(directory_path, ending)):
...
def compute_weight(train_masks_reshaped_encoded, n_classes, class_weights='balanced'):
...
def get_model(X_train, X_test, y_train_cat, y_test_cat, n_classes, dropout, weights, class_weights,
    ...
    model = single_unet_model(n_classes=n_classes, IMG_HEIGHT=IMG_HEIGHT, IMG_WIDTH=IMG_WIDTH, IMG_CHANNELS=IMG_CHANNELS, dropout=dropout)
...
def train_and_evaluate_models(model_names, train_images, train_masks, X_train, X_test, X_test_unnormalized, y_train_cat, y_test_cat, y_test, test_img_number, n_classes, class_weights, verbose=1, epochs=50):
    ...
    for i, model_name in enumerate(model_names):
        ...
        IOU(train_images, train_masks, model, X_test, y_test)
...
def IOU(train_images, train_masks, model, X_test, y_test):
    # IOU
    y_pred = model.predict(X_test)
    y_pred_argmax = np.argmax(y_pred, axis=3)

    IOU_keras = MeanIoU(num_classes=2)  # Assuming 2 classes
...
if __name__ == '__main__':
    ...
    # Input folders containing labeled images - Resized
    resized_ship_folder = r"/home/nmammadli/Downloads/Train_val2017/Labeled_train_val2017"
    resized_output_folder = r"/home/nmammadli/Downloads/Train_val2017"
    
    # Color & Resized Model names
    model_name17 = r"Models/Resized/Single_UNet/Color/resized_SingleUNet_color.hdf5"
    model_name18 = r"Models/Resized/Single_UNet/Color/resized_SingleUNet_color_dropout.hdf5"
    model_name19 = r"Models/Resized/Single_UNet/Color/resized_SingleUNet_color_weights.hdf5"
    model_name20 = r"Models/Resized/Single_UNet/Color/resized_SingleUNet_color_weights_dropout.hdf5"
    
    # Gray & Resized Model names
    model_name21 = r"Models/Resized/Single_UNet/Gray/resized_SingleUNet_gray.hdf5"
    model_name22 = r"Models/Resized/Single_UNet/Gray/resized_SingleUNet_gray_dropout.hdf5"
    model_name23 = r"Models/Resized/Single_UNet/Gray/resized_SingleUNet_gray_weights.hdf5"
    model_name24 = r"Models/Resized/Single_UNet/Gray/resized_SingleUNet_gray_weights_dropout.hdf5"
    
    model_names5 = [model_name17, model_name18]#, model_name19, model_name20]
    model_names6 = [model_name21, model_name22]#, model_name23, model_name24]
    
    #Resizing images, if needed
    test_img_number = 11 #11, 13, 42
    epochs = 50
    SIZE_X = 256 
    SIZE_Y = 256
    n_classes = 2
    ...
    label_color = {"ship": (116, 244, 255)}

    train_masks_resized = get_label(resized_ship_folder, resized_output_folder, label_color, color_mask=False, save=False, SIZE_X=SIZE_X, SIZE_Y=SIZE_Y)
    
    img_path = r"/home/nmammadli/Downloads/Train_val2017/train_val2017"
    mask_path = r"/home/nmammadli/Downloads/Train_val2017"
    ...
    train_masks_encoded_original_shape, train_masks_reshaped_encoded = encode_labels(train_masks_resized)
    ...
    class_weights = compute_weight(train_masks_reshaped_encoded, n_classes, class_weights='balanced')
    
    # test_img_number = random.randint(0, len(X_test_unnormalized_color))
    models_color, historyies_color, times_color = train_and_evaluate_models(model_names5, train_images_color, train_masks_resized, X_train_color, X_test_color, X_test_unnormalized_color, y_train_cat_color, y_test_cat_color, y_test_color, test_img_number, n_classes, class_weights, epochs=epochs)
    models_gray, historyies_gray, times_gray = train_and_evaluate_models(model_names6, train_images_gray, train_masks_resized, X_train_gray, X_test_gray, X_test_unnormalized_gray, y_train_cat_gray, y_test_cat_gray, y_test_gray, test_img_number, n_classes, class_weights, epochs=epochs)